...
{loading && <Loader />}
      ...
      <div className="relative mb-4">
        ...
        <input onChange={(e) => setEmail(e.target.value)} placeholder='Enter Email' value={email} type="email" id="email" name="email" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
      </div>
      <div className="relative mb-4">
      ...
      <p className='my-5'>New Customer ? <Link className='font-medium' to={redirect ?  `/register?redirect=${redirect}`: '/register'}>Register</Link>  </p>
    </div>
  </div>
  </div>
</section>
